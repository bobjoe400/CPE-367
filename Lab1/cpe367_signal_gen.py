...
def gen_wav(fpath_wav_out):
	...
	num_samples = 8000 #change numsamples to 1000 so we have 1/16th of a second of 16kHz of samples
	w1 = 2*math.pi*1000/sample_rate_hz #same frequency formula but using 160hz
	A1 = (1<<15) - 1 #max amplitude is the same as normal 
	# students - well done!
	###############################################################
	###############################################################
	...
	for n in range(num_samples):
		...
		if n>=800:
			sigma = 347.436
			mag = A1*math.exp(-(n-800)/sigma)
			yout = mag*math.sin(w1*n)
		else: yout = 0
...
def main():
	...
	fpath_wav_out = 'testimpulsedelay.wav'
